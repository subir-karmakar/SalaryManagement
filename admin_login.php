...
if(isset($_POST["submit"]))
{
   ...
   $pass=$_SESSION['pass'];
    $query="SELECT * FROM `adm` where uname='$uname' and pass='$pass'";
    $res=mysqli_query($conn,$query);
    $rows=mysqli_num_rows($res);
    if($rows==1){
      echo "<script>
      location.replace('adm_home.php');
      </script>";
    }
    else{
      echo "<script>
	...
	$unsql="SELECT `uname` FROM `adm` WHERE `uname`='$unname'";
...
<body id="page-top" style="height: 100%;">
    ...
    <div class="login-dark" style="background-image: url(&quot;assets/img/laptop2.jpg&quot;);background-repeat: no-repeat;background-size: cover;background-position: center;height: 911px;">
        ...
        <form class="text-center" method="post" style="background-color: rgba(255,255,255,0.78);padding-top: 20px;padding-bottom: 20px;padding-right: 30px;width: 300px;padding-left: 30px;margin-top: -98px;"><label class="text-left" style="color: rgb(90,92,105);font-size: 20px;font-weight: normal;margin-top: 0px;">Admin Login</label>
            ...
            <div class="form-group"><input class="form-control" type="text" name="uname" placeholder="Enter User ID" required="" style="color: rgb(90,92,105);"></div>
