...
if(isset($_POST["submit"]))
{
    ...
    $dupsql="SELECT * from `bonus_form` WHERE `empid`='$empid' AND `month`='$month' AND `year`='$year' ";
    $duplicate=mysqli_query($conn,$dupsql);
    if(mysqli_num_rows($duplicate)>0)
    {
      echo "<script>
      alert('Data is already submitted');
      location.replace('normal_bonus.php?');
      </script>";
    }

    else{
    
	$sql="INSERT INTO `bonus_form`(`empid` ,`month`, `rate` ,`year`) VALUES ('$empid','$month','$rate','$year')";
    ...
    <div id="wrapper">
        <div class="d-flex flex-column" id="content-wrapper" style="height: 100%;">
                    ...
                    <div class="collapse navbar-collapse text-center" id="navcol-1">
                        <ul class="nav navbar-nav text-center mx-auto" style="margin: 0px;margin-left: 0px;width:100%;padding: 0px;padding-right: 0px;padding-left: 0px;">
                            ...
                            <li class="nav-item dropdown" style="margin-right: 15px;"><a class="dropdown-toggle nav-link" data-toggle="dropdown" aria-expanded="false" href="normal_bonus.php">Set Bonus</a>
                                <div class="dropdown-menu" role="menu"><a class="dropdown-item active" role="presentation" href="normal_bonus.php">Normal Bonus</a><a class="dropdown-item" role="presentation" href="fix_bonus.php">Occasional Bonus</a></div>
                            </li>
            ...
            <div class="login-dark" style="background-image: url(&quot;assets/img/laptop2.jpg&quot;);height: 860px;">
                <form class="text-center" method="post" style="background-color: rgba(255,255,255,0.78);padding-top: 20px;padding-bottom: 20px;padding-right: 30px;padding-left: 30px;margin-top: 0px;"><label class="text-left" style="color: rgb(90,92,105);font-size: 20px;font-weight: normal;margin-top: 0px;">Set Normal Bonus</label>
                    ...
                    <div class="form-group"><label style="color: rgb(90,92,105);">EMP ID</label><select class="custom-select" style="color: rgb(90,92,105);" required="" name="empid"><option  disabled value = "" selected>~ Select EMP ID ~</option><?php
                  $tsql="SELECT `empid` FROM `emp`";
                  $trec=mysqli_query($conn,$tsql);
                  while($trows=mysqli_fetch_array($trec)){
                  ?>
                  <option value="<?php echo $trows['empid']?>">
                  <?php echo $trows['empid']?>
                  </option>
                  <?php
                  }
                  ?></select></div>
                  ...
                  ?></select></div>
                    <div class="form-group"><label style="color: rgb(90,92,105);">Rate (%)</label><input class="form-control" type="number" style="color: rgb(90,92,105);" placeholder="Enter Bonus Rate" name="rate" required=""></div>
                    <div class="form-group"><button class="btn btn-primary btn-block" type="submit" name="submit" style="background-color: rgb(55,55,55);">Set Bonus</button></div>
                </form>
            </div>
